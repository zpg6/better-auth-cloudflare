...
async function resolveCloudflareGeolocation(
    source: CloudflarePluginOptions["cf"]
): Promise<CloudflareGeolocation | null | undefined> {
...
function assertAtomicStorageCompatibility(version: string, options: BetterAuthOptions): void {
    ...
    if (parsed?.[0] === 1 && parsed[1] < 6 && customStorage && !legacyRateLimitStorage) {
        throw new Error(
    ...
    if (!usesAtomicContract || !options.secondaryStorage) return;

    const storage = options.secondaryStorage as SecondaryStorage & AtomicSecondaryStorage;
    const problems: string[] = [];

    if (options.verification?.storeInDatabase === true) {
        if (!options.database) {
            problems.push("verification.storeInDatabase requires a database");
        }
    } else if (typeof storage.getAndDelete !== "function") {
        problems.push("verification requires a database or secondaryStorage.getAndDelete");
    }

    if (options.rateLimit?.enabled !== false) {
        if (customStorage) {
            if (typeof customStorage.consume !== "function") {
                problems.push("rateLimit.customStorage requires an atomic consume function");
            }
        } else {
            const rateLimitStorage = options.rateLimit?.storage ?? "secondary-storage";
            if (rateLimitStorage === "database" && !options.database) {
                problems.push('rateLimit.storage "database" requires a database');
            } else if (rateLimitStorage === "secondary-storage" && typeof storage.increment !== "function") {
                problems.push("secondary-storage rate limiting requires secondaryStorage.increment");
            }
        }
    }
    ...
    if (problems.length > 0) {
        throw new Error(
            `Better Auth ${version} storage is not atomic: ${problems.join("; ")}. ` +
                "Use database routing or the Durable Object adapters from better-auth-cloudflare."
        );
    }
}
 ...
 */
export const withCloudflare = <const T extends BetterAuthOptions>(
