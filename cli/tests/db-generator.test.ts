...
describe("Database Generator", () => {
    ...
    describe("Next.js Template - MySQL/Hyperdrive", () => {
        test("generates Next.js MySQL database index with default binding", () => {
            ...
            expect(result).toContain("export async function getDb()");
            expect(result).toContain("mysql.createConnection({");
            expect(result).toContain("host: env.HYPERDRIVE.host");
    ...
    describe("Function Generation", () => {
        test("generates appropriate connection logic for each database", () => {
            ...
            // MySQL should use a Worker-compatible connection
            expect(mysqlResult).toContain("const connection = await mysql.createConnection({");
            expect(mysqlResult).toContain("disableEval: true");
            expect(mysqlResult).toContain("return drizzle(connection, {");
        });
    });
